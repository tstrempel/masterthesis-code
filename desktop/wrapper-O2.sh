...
function run_pipeline {
    ...
    sleep $4
    { time sorting-O2 "$2" "$3"; } 2> "$1/log.txt" &
    processid_sorting=$!
...
function run_pipeline_vtune {
    ...
    processid_scaphandre=$!
    sleep $4
    { vtune -collect hotspots -knob sampling-mode=hw -knob sampling-interval=0.5 -r "$1/hs_$2_$3" sorting-O2 "$2" "$3" 2>&1 "$1/log.txt"; } > "$1/log_vtune.txt" &
    processid_sorting=$!
...
# alias scaphandre='docker run -v /sys/class/powercap:/sys/class/powercap -v /proc:/proc -ti tstrempel/scaphandre'
