...
function run_pipeline {
    ...
    sleep $4
    { time sorting-O3 "$2" "$3"; } 2> "$1/log.txt" &
    processid_sorting=$!
    ...
    python evaluation.py "$1/energy_data_beautified.json" "$1" "$5" 15 4 'sorting-O3' >> "$1/log.txt"
}
...
function run_pipeline_vtune {
    ...
    processid_scaphandre=$!
    sleep $4
    { vtune -collect hotspots -knob sampling-mode=hw -knob sampling-interval=0.5 -r "$1/hs_$2_$3" sorting-O3 "$2" "$3" 2>&1 "$1/log.txt"; } > "$1/log_vtune.txt" &
    processid_sorting=$!
