...
plt.figure("vtune and vpxenc")
plt.plot(energy_data['timestamp'], energy_data['consumption'], label="Total power consumption")
plt.plot(apps['vpxenc']['timestamp'], apps['vpxenc']['consumption'], label="vpxenc")
plt.plot(apps['vtune']['timestamp'], apps['vtune']['consumption'], label="vtune")
plt.xticks([energy_data['timestamp'][0], energy_data['timestamp'][len(energy_data)-1]])
plt.ylim(0, 16)
plt.axhline(y=15, color='r', linestyle='-', label='TDP of 15W')
plt.legend()
plt.grid()
print("vpxenc")
print(consumption_per_app.loc[consumption_per_app['app_name'] == "vpxenc"]['consumption'])
print("vtune")
print(consumption_per_app.loc[consumption_per_app['app_name'] == "vtune"]['consumption'])
